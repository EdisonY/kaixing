;(function(global, undefined) {
    "use strict"
    var _global,panZoom,state,time,func,hot = [],arrowHref = 'http://172.16.1.110/subway/arrow.png',turnHref= 'http://172.16.1.110/subway/turn.png',Num = 0,stationPath = [];
    ...
    function tct_subway(opt){
        var self = this
        // loadJS('http://172.16.1.110/subway/jquery.min.js',function(){
            loadJS('http://172.16.1.110/subway/svg-pan-zoom.min.js',function(){
                self._initial(opt);
            })
        // });
    }
    ...
    tct_subway.prototype = {
        ...
        _initial:function(opt){
            ...
            $.ajax({
                // url: "beijing.xml",
                url: "http://172.16.1.110/subway/beijing.xml?" + Math.floor(Math.random()*1000),
                dataType: 'xml',
                type: 'GET',
                timeout: 5000,
                error: function(xml){
        ...
        _getLine:function(){
            ...
            this._generate('image',{'href':'http://172.16.1.110/subway/tam.png','width':85,'height':26,'x':908,'y':758,'class':'tam'},'image')
            this._generate('image',{'href':'http://172.16.1.110/subway/north.png','width':50,'height':100,'x':1710,'y':250,'class':'cluster'},'image')
            this._generate('image',{'href':'http://172.16.1.110/subway/airport.png','width':40,'height':40,'x':1510,'y':355,'class':'cluster'},'image')
            this._generate('image',{'href':'http://172.16.1.110/subway/airport.png','width':40,'height':40,'x':1000,'y':1228,'class':'cluster'},'image')
            this._generate('image',{'href':'http://172.16.1.110/subway/birde.png','width':80,'height':40,'x':940,'y':480,'class':'cluster'},'image')
            this._generate('image',{'href':'http://172.16.1.110/subway/jump.png','width':45,'height':40,'x':248,'y':770,'class':'cluster'},'image')
        ...
        drewAlarm:function(from,state){
            ...
            this._generate('image',{'href':'http://172.16.1.110/subway/' + state + '.png','width':30,'height':40,'x':from.line.length > 1 ? from.x - 18 : from.x - 15,'y':from.y - 50,'class':'cluster zk ' + cname},'Passengerflow')
        },
        drewStationAlarm:function(from,state){
            ...
            this._generate('image',{'href':'http://172.16.1.110/subway/' + state + '.png','width':30,'height':40,'x':from.line.length > 1 ? from.x - 18 : from.x - 15,'y':from.y - 50,'class':'cluster zk szk ' + cname,type:'zk'},'Passengerflow')
        },
        ...
        drewRunning:function(from,to,trainId,Percent,arrive,direction,speed){
            ...
            if($('#' + trainId + 'realtime').length > 0){
                ...
                this._generate('image',{id:trainId + 'realtime','href':'http://172.16.1.110/subway/type1.png','width':10,'height':10,'class':direction == 'down' ? 'cluster train fixgap ' + cname : 'cluster train ' + cname},'alarm')
        ...
        drewStop:function(station,trainId,lineid){
            if($('#' + trainId + 'stop').length == 0 && station && station.line){
                this._generate('image',{id:trainId + 'stop','href':'http://172.16.1.110/subway/type2.png','width':10,'height':10,'x':station.line.length > 1 ? station.x - 3 : station.x,'y':station.line.length > 1 ? station.y - 3 : station.y,'class':'cluster train ' + lineid},'stop')
            }
        },
        clearStop:function(){
            $('#stop').html('')
