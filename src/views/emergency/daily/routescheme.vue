<template>
    <div class="routescheme">
        ...
        <div style="margin: 0 0 0 20px">
            ...
            <el-button @click="sc('开行方案生成成功')"
                type="primary">生成开行方案</el-button>
            <el-button type="primary">导入历史方案</el-button>
        </div>
        <el-dialog title="添加条数"
        ...
        <el-dialog title="选择交路方案详情"
            ...
            <div class="imitate">
                ...
                <p v-for="item in detailsData"
                    :key="item.data">
                    <strong v-if="item.name">
                        <el-radio v-model="radio"
                            :label="item.date"></el-radio>
                        <!-- {{item.date}} -->
                    </strong>
                    <strong v-if="item.name">{{item.name}}</strong>
                    <strong v-if="item.name"
                        style="width:28%">{{item.address}}</strong>
                    ...
                    <strong v-if="item.children">
                        <b v-for="items in item.children"
                            :key="items">{{items.name}}</b>
                    </strong>
                    <strong v-if="item.children"
                        style="width:28%">
                        <b v-for="items in item.children"
                            :key="items">{{items.address}}</b>
                    </strong>
                    <strong v-if="item.children">
                        <b v-for="items in item.children"
                            :key="items">{{items.station}}</b>
                    </strong>
                    <strong v-if="item.children">
                        <b v-for="items in item.children"
                            :key="items">{{items.message}}</b>
                    </strong>
                    <strong v-if="item.children"
                        style="line-height:25px">{{item.bili}}</strong>
                </p>
            </div>
...
.detailDia .el-table__body tr.current-row > td {
    background: #fed24b;
}
